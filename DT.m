...
for f = 1:fold_number
    ...
    %For Decision Tree method fitctree model is used 
    for min_leaf_size = hypparameters.MinLeafSize
        for max_split_number = hypparameters.MaxNumSplits
            %Applying the current hyperparameters on fitctree model and predicting on validation data set 
            model = fitctree(x_second_train, y_second_train, ...
            ...
            if model_accuracy > resultacc
                ...
                tuned_model = model;
                resulthypparameters = struct('MaxNumSplits', max_split_number, 'MinLeafSize', min_leaf_size);
            end
        end
    end
end
...
tuned_model = fitctree(x_train, y_train, ...
                'MaxNumSplits', resulthypparameters.MaxNumSplits, ...
                'MinLeafSize', resulthypparameters.MinLeafSize ...
                );
...
confusionchart(y_test, tuned_predicted, 'ColumnSummary', 'column-normalized', ...
    'RowSummary', 'row-normalized', 'DiagonalColor', 'green');
title ('Gender Classification (DT)');
TP = dt_conf(1, 1);
FP = dt_conf(1, 2);
FN = dt_conf(2, 1);
Precision = TP / (TP + FP);
